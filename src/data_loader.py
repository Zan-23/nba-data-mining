...
def load_data(columns=None, seasons=None, path=RAW_DATA_PATH, resolve=True):
    ...
    for file_name in files:
        ...
        if resolve:
            if columns is None or "EVENTMSGTYPE" in columns:
                season["EVENTMSGTYPE"] = season["EVENTMSGTYPE"].replace(EVENTMSGTYPE_dict)
            if columns is None or "EVENTMSGACTIONTYPE" in columns:
                season.loc[season["EVENTMSGTYPE"] == EVENTMSGTYPE_dict[1], "EVENTMSGACTIONTYPE"] = season.loc[
                    season["EVENTMSGTYPE"] == EVENTMSGTYPE_dict[1], "EVENTMSGACTIONTYPE"].replace(
                    EVENTMSGACTIONTYPE_FIELD_GOAL_dict)
            if columns is None or "EVENTMSGACTIONTYPE" in columns:
                season.loc[season["EVENTMSGTYPE"] == EVENTMSGTYPE_dict[2], "EVENTMSGACTIONTYPE"] = season.loc[
                    season["EVENTMSGTYPE"] == EVENTMSGTYPE_dict[2], "EVENTMSGACTIONTYPE"].replace(
                    EVENTMSGACTIONTYPE_FIELD_GOAL_dict)
            if columns is None or "EVENTMSGACTIONTYPE" in columns:
                season.loc[season["EVENTMSGTYPE"] == EVENTMSGTYPE_dict[3], "EVENTMSGACTIONTYPE"] = season.loc[
                    season["EVENTMSGTYPE"] == EVENTMSGTYPE_dict[3], "EVENTMSGACTIONTYPE"].replace(
                    EVENTMSGACTIONTYPE_FREE_THROW_dict)
