...
def load_game_data(columns=None, seasons=None, path=RAW_DATA_PATH):
    ...
    for pbp_data in pbp_data_per_season:
        ...
        game_data["visitor_scoring_leader"] = pbp_grouped.apply(lambda x: (
            (x[(x["EVENTMSGTYPE"] == 'FIELD_GOAL_MADE') & (x["VISITORDESCRIPTION"].str.contains('3PT')) & (x["PERSON1TYPE"] == 'VISITOR_PLAYER')].groupby("PLAYER1_ID").size() * 3).add(
            x[(x["EVENTMSGTYPE"] == 'FIELD_GOAL_MADE') & ((x["VISITORDESCRIPTION"].str.contains('3PT'))==False) & (x["PERSON1TYPE"] == 'VISITOR_PLAYER')].groupby("PLAYER1_ID").size() * 2, fill_value=0).add(
            x[(x["EVENTMSGTYPE"] == 'FREE_THROW') & ((x["VISITORDESCRIPTION"].str.contains('MISS'))==False) & (x["PERSON1TYPE"] == 'VISITOR_PLAYER')].groupby("PLAYER1_ID").size(), fill_value=0)
        ).idxmax())
...
def load_player_data(columns=None, seasons=None, path=RAW_DATA_PATH, player_info_path = PLAYER_INFO_PATH):
    ...
    for pbp_data in pbp_data_per_season:
        ...
        player_data = player_data.merge(player_info[player_info["Season"]==pbp_data.iloc[0]["season_name"]].set_index("Player ID")[["Age","Height","Weight","College","Country","Draft Year","Draft Number","Draft Number"]], left_index=True, right_index=True)
        ...
        #Field goal stats:
        player_data["fg_made"] = pbp_grouped_pID1.apply(lambda x: (x["EVENTMSGTYPE"] == 'FIELD_GOAL_MADE') & (x["PLAYER1_ID"] == x.name)).groupby(level=0).sum()
        player_data["fg_made"] = player_data["fg_made"].fillna(0).astype(int)
        player_data["fg_missed"] = pbp_grouped_pID1.apply(lambda x: (x["EVENTMSGTYPE"] == 'FIELD_GOAL_MISSED') & (x["PLAYER1_ID"] == x.name)).groupby(level=0).sum()
        player_data["fg_missed"] = player_data["fg_missed"].fillna(0).astype(int)
        player_data["3PT_made"] = pbp_grouped_pID1.apply(lambda x: ((x["EVENTMSGTYPE"] == 'FIELD_GOAL_MADE') & (x["PLAYER1_ID"] == x.name)) & ((x["VISITORDESCRIPTION"].str.contains("3PT").fillna(False)) | (x["HOMEDESCRIPTION"].str.contains("3PT").fillna(False)))).groupby(level=0).sum()
        player_data["3PT_made"] = player_data["3PT_made"].fillna(0).astype(int)
        player_data["3PT_missed"] = pbp_grouped_pID1.apply(lambda x: ((x["EVENTMSGTYPE"] == 'FIELD_GOAL_MISSED') & (x["PLAYER1_ID"] == x.name)) & ((x["VISITORDESCRIPTION"].str.contains("3PT").fillna(False)) | (x["HOMEDESCRIPTION"].str.contains("3PT").fillna(False)))).groupby(level=0).sum()
        player_data["3PT_missed"] = player_data["3PT_missed"].fillna(0).astype(int)

        #Free throw stats:
        player_data["ft_made"] = pbp_grouped_pID1.apply(lambda x: ((x["EVENTMSGTYPE"] == 'FREE_THROW') & (x["PLAYER1_ID"] == x.name)) & (~((x["VISITORDESCRIPTION"].str.contains("MISS").fillna(False)) | (x["HOMEDESCRIPTION"].str.contains("MISS").fillna(False))))).groupby(level=0).sum()
        player_data["ft_made"] = player_data["ft_made"].fillna(0).astype(int)
        player_data["ft_missed"] = pbp_grouped_pID1.apply(lambda x: ((x["EVENTMSGTYPE"] == 'FREE_THROW') & (x["PLAYER1_ID"] == x.name)) & ((x["VISITORDESCRIPTION"].str.contains("MISS").fillna(False)) | (x["HOMEDESCRIPTION"].str.contains("MISS").fillna(False)))).groupby(level=0).sum()
        ...
        #Rebound stats:
        player_data["rebounds"] = pbp_grouped_pID1.apply(lambda x: ((x["EVENTMSGTYPE"] == 'REBOUND') & (x["PLAYER1_ID"] == x.name))).groupby(level=0).sum()
        player_data["rebounds"] = player_data["rebounds"].fillna(0).astype(int)

        #Assist stats:
        player_data["assists"] = pbp_grouped_pID2.apply(lambda x: ((x["EVENTMSGTYPE"] == 'FIELD_GOAL_MADE') & (x["PLAYER2_ID"] == x.name))).groupby(level=0).sum()
        player_data["assists"] = player_data["assists"].fillna(0).astype(int)

        #Turnover stats:
        player_data["turnover"] = pbp_grouped_pID1.apply(lambda x: ((x["EVENTMSGTYPE"] == 'TURNOVER') & (x["PLAYER1_ID"] == x.name))).groupby(level=0).sum()
        player_data["turnover"] = player_data["turnover"].fillna(0).astype(int)

        #Foul stats: (not just personal fouls)
        player_data["fouls"] = pbp_grouped_pID1.apply(lambda x: ((x["EVENTMSGTYPE"] == 'FOUL') & (x["PLAYER1_ID"] == x.name))).groupby(level=0).sum()
